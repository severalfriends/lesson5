...
pub fn process_instruction(
    ...
    let instruction: NoteInstruction = NoteInstruction::unpack(instruction_data)?;
    match instruction {
...
pub fn create_note(
    ...
    let noter = next_account_info(account_info_iter)?;
    ...
    let rent_lamports: u64 = rent.minimum_balance(NoteAccountState::MAX_SIZE);
    ...
    invoke(
        ...
        &[noter.clone(), note.clone(), system_program.clone()],
    )?;
...
pub fn update_node(
    ...
    let mut note_data: NoteAccountState = try_from_slice_unchecked::<NoteAccountState>(&note.data.borrow()).unwrap();
    if *noter.key != note_data.authority {
...
pub fn delete_note(_program_id: &Pubkey, accounts: &[AccountInfo])-> ProgramResult {
    ...
    let note_data: NoteAccountState = try_from_slice_unchecked::<NoteAccountState>(&note.data.borrow()).unwrap();

    if *noter.key != note_data.authority {
        return Err(NoteError::InvalidNoteAuthority.into())
    }
    let noter_lamports= noter.lamports();
    **noter.lamports.borrow_mut() = noter_lamports.checked_add(note.lamports()).unwrap();
